...
Deep Learning(DL) methods allow direct inference of features from inputs. These methods have demonstrated success in a variety of fields, such as computer vision and natural language processing. In recent years, there has been significant progress with DL methods in the drug discovery field with a focus on convolutional neural networks(CNN). CNNs, typically applied to images, allow for limited preprocessing of the inputs and infer features from the images presented to them. CNNs have shown potential in virtual screening (AtomNet, DeepVS, \cite{Ragoza2017}) and binding affinity prediction (PotentialNet, $K_{DEEP}$, Fafnucy).
